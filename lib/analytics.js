 ...
 * - data: Flexible object with event-specific metadata — identifiers only
 *   (orgUuid/userId/cardId/etc.); never full card content (href/title/description)
 *   or other PII (see issue #13's security constraint).
 *
 * Usage:
 *   import { logEvent, EVENT_TYPES } from '@/lib/analytics';
 *   logEvent(EVENT_TYPES.CARD_CLICK, { cardId, orgUuid, userId, metadata... });
 *
 * Current call sites (wired in, issue #13):
 * - pages/api/cards/index.js (POST)              -> CARD_CREATE
 * - pages/api/cards/[id].js (PATCH, DELETE)       -> CARD_UPDATE, CARD_DELETE
 * - pages/api/cards/reorder.js                    -> CARD_REORDER
 * - pages/api/admin/users/[ssoUserId]/change-role.js  -> logAdminAction('change_role', ...)
 * - pages/api/admin/users/[ssoUserId]/grant-access.js -> logAdminAction('grant_access', ...)
 * - pages/api/admin/users/[ssoUserId]/revoke-access.js -> logAdminAction('revoke_access', ...)
 * - pages/api/oauth/callback.js (successful login) -> USER_LOGIN (additive, alongside the
 *   existing recordAuthEvent audit-trail call — analyticsEvents is for usage analytics,
 *   authLogs remains the separate security audit trail)
 *
 * Deliberately deferred (not wired in by issue #13, tracked as future work):
 * - logCardClick / CARD_CLICK: card clicks are a client-side navigation with no server
 *   round-trip today; wiring this needs a new beacon endpoint (POST /api/analytics/click),
 *   which is a materially larger feature (public endpoint, abuse/rate-limit surface).
 * - ORG_CREATE/UPDATE/DELETE: pages/api/organizations/** is not touched by this module yet.
 *
 * Known limitation — best-effort delivery, not exactly-once:
 * This module's batch queue is in-memory and per-process. A partial batch (fewer than
 * BATCH_SIZE events, less than BATCH_INTERVAL_MS since the last flush) can be lost if the
 * process terminates abruptly — e.g. a serverless function's cold shutdown outrunning the
 * SIGTERM/SIGINT handlers below. This is an acknowledged, accepted trade-off of the
 * batching design (favoring reduced DB load over guaranteed delivery), not a defect to
 * redesign here: analytics data is expected to be near-complete, not authoritative — use
 * authLogs (recordAuthEvent) for anything that requires guaranteed, synchronous delivery.
 *
 ...
 * @param {Record<string, any>} [data] - Event-specific fields (orgUuid, userId, and any
 *   number of free-form identifier fields per event type — cardId, action, etc; see the
 *   file-header "Schema" note above for the identifiers-only/no-PII constraint). Kept
 *   as a loose bag rather than a fixed shape: real call sites across pages/api/**
 *   pass different extra fields per event type directly on this object (not nested
 *   under a `metadata` key, despite this doc's pre-issue-#16 wording implying that —
 *   corrected here to match actual usage).
 *
 * Why async?
 ...
 *   });
 */
export function logEvent(eventType, data = /** @type {Record<string, any>} */ ({})) {
  // Validate event type
