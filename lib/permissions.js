...
export function getPermissionMetrics() {
  // Functional: both branches now return a string (was `0` as a bare number in the
  // false branch) so parseFloat() below always receives a string, matching its
  // declared signature; parseFloat('0.00') === 0, so this is not a behavior change.
 ...
 * @returns {Promise<{permissions: Set<string>}|null>} Role object with permissions Set, or null
 */
export async function getOrgRole(orgUuid, roleId) {
  ...
  try {
    ...
    // Boundary cast — driver findOne() result is loosely typed; project only
    // {permissions} per the query above, so the assertion is narrower than UserDoc/etc.
    const role = /** @type {{permissions?: string[]} | null} */ (await col.findOne(
 ...
 *
 * @param {UserDoc | null | undefined} user - User object with ssoUserId, email, and isSuperAdmin fields
...
export async function getUserOrgRole(ssoUserId, orgUuid) {
  ...
  try {
    ...
    const membership = /** @type {{role?: 'admin'|'user'} | null} */ (await col.findOne(
 ...
 *
 * @param {UserDoc | null | undefined} user - Authenticated user object with ssoUserId and isSuperAdmin
 ...
 * @param {(import('http').IncomingMessage & { _permissionCache?: Map<string, string|null> }) | null} [req] - Optional request object for per-request caching
