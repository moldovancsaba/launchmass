 ...
 * Strategic: Creates or updates local user record; sets isAdmin=true ONLY on insert (auto-admin requirement);
 * preserves isAdmin and localPermissions on updates to enable future manual permission management
 * 
 * Why isAdmin is $setOnInsert: Initial requirement is auto-admin for all SSO users, but we must not
 * overwrite manual admin revocations in future when permission management is added
 * 
 * Timestamp format: ISO 8601 with milliseconds in UTC per project policy
 * 
 * @param {Object} ssoUser - User object from SSO validate endpoint { id, email, name, role, permissions }
...
export async function upsertUserFromSso(ssoUser) {
  ...
  // Strategic: $setOnInsert for fields that should never be overwritten (isAdmin, localPermissions, createdAt)
  // $set for fields that should sync from SSO on every login (email, name, ssoRole, timestamps)
  const update = {
    $setOnInsert: {
      ssoUserId: ssoUser.id,
      // Strategic: Auto-admin requirement — all SSO users get admin on first login
      isAdmin: true,
      // Functional: Empty permissions object ready for future granular permission system
      localPermissions: {},
      createdAt: now,
    },
    $set: {
