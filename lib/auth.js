/**
 * ⚠️ DEPRECATED - LEGACY AUTHENTICATION MODULE ⚠️
 * 
 * This module uses the legacy cookie-forwarding SSO authentication approach.
 * It has been superseded by OAuth 2.0 authentication in lib/auth-oauth.js.
 * 
 * Migration Path:
 * - For new code: Import from 'lib/auth-oauth.js' instead
 * - For existing code: See DEPRECATED_AUTH.md for migration guide
 * - Timeline: This module will be removed in v2.0.0
 * 
 * Why Deprecated:
 * - OAuth 2.0 is the standard modern authentication protocol
 * - Token-based authentication is more secure than cookie forwarding
 * - Direct token validation avoids extra network roundtrip to SSO server
 * 
 * Functional: SSO session validation and authentication middleware (LEGACY)
 * Strategic: Cookie forwarding approach; replaced by OAuth 2.0 token validation
 */
