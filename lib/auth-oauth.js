...
/**
 * Functional: Collapse a Node request header value to a single string.
 * Strategic: Node's http typings allow a small set of headers (including
 * x-forwarded-for) to be `string | string[]`, since proxies may fold repeated
 * headers into an array — this repo's audit-log `ip` field is a plain string, so
 * this picks the first value. Not previously needed at runtime (`||` coerced fine),
 * but needed here so recordAuthEvent's `event.ip: string` JSDoc holds under checkJs.
 *
 ...
 * 
 * @param {import('http').IncomingMessage} req - Next.js request object with headers
 * @returns {Promise<{isValid: boolean, user?: import('./types.js').UserDoc}>}
 */
  ...
  try {
    ...
    if (!sessionCookie) {
      ...
      await recordAuthEvent({
        ...
        ip: firstHeaderValue(req.headers['x-forwarded-for']) || req.connection?.remoteAddress || 'unknown',
 ...
 *
 * @param {import('./types.js').UserDoc | null | undefined} user - req.user / SSO session user (as attached by withSsoAuth / validateSsoSession)
 * @returns {boolean}
 */
