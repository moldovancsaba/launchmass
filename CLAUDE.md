...
- **The `create_pull_request` MCP tool shares that same auto-append behavior on PR
  creation** (confirmed on PR #31: a clean, footer-free `body` was submitted, and the
  same `\n\n---\n_Generated by [Claude Code](...)_` footer, complete with a
  session-link URL, appeared in the created PR's description) — but unlike the
  comment-reply tool, **`update_pull_request` is a working workaround here**: calling
  it with the identical clean body afterward replaces the description in place and
  does *not* re-trigger the auto-append (confirmed by re-reading the PR body after the
  update call and finding no footer). So for PRs specifically: create normally, then
  immediately re-read the PR body, and if the footer is present, fix it with one
  `update_pull_request` call passing the same clean body back — this is a real fix
  here, not merely a mitigation, unlike the comment-reply case above where no such
  workaround exists.
