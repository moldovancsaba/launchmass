...
- **`create_pull_request` also auto-appends the same footer.** Confirmed on PR #40: the
  `body` passed at creation had no footer (verifiable — it's this session's own text,
  not reconstructed from memory), but the PR's body as returned by a later
  `pull_request_read` had `\n\n---\n_Generated by [Claude Code](...)_` appended. Same
  fix as the reply-comment case: `update_pull_request` with the clean body immediately
