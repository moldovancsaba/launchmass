...
function Row({ item, onChange, onDelete }) {
  ...
  const style = { transform: CSS.Transform.toString(transform), transition, display: 'grid', gridTemplateColumns: '24px 1fr 1fr 2fr 120px', gap: 8, alignItems: 'center', padding: 8, background: 'rgba(255,255,255,0.9)', borderRadius: 12, border: '1px solid #e6e8ef' };
  return (
    <div ref={setNodeRef} style={style}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={() => onChange(item, true)}>Save</button>
...
export default function AdminCards() {
  ...
  async function saveItem(updated, forceSave = false) {
    try {
      const res = await fetch('/api/cards/' + updated._id, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ title: updated.title, href: updated.href, description: updated.description })
      });
      if (!res.ok) throw new Error();
      setStatus('Saved'); setTimeout(() => setStatus(''), 1200);
    } catch { setStatus('Save failed'); setTimeout(() => setStatus(''), 1500); }
  }

  async function deleteItem(id) {
    try {
      const res = await fetch('/api/cards/' + id, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } });
      if (!res.ok) throw new Error();
  ...
  async function addItem() {
    try {
      const res = await fetch('/api/cards', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ title: 'New Card', href: 'https://example.com', description: 'Describe me' })
      });
      const created = await res.json();
      if (res.ok) setItems([...items, created]);
    } catch {}
  }
  ...
  return (
    <main className="grid" style={{ gridTemplateColumns: '1fr', rowGap: 12 }}>
      ...
      <section style={{ gridColumn: '1 / -1', opacity: .7, fontSize: 12 }}>
        Drag rows to reorder. Use Save on a row to update its fields. Delete removes the card. Add new creates a default card.
      </section>
    </main>
  );
}
