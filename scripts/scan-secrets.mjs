...
// Functional: exact-value allow-list for intentional placeholder/fixture content in
// specific files, applied only to the default (staged-diff or full-tracked-tree) scan
// modes — see main(). Deliberately NOT a filename allow-list: skipping a whole file by
// name would also hide a real credential accidentally pasted into that same file
// (e.g. alongside or in place of the placeholder), which defeats the guard's purpose.
// Instead, only the exact known placeholder/fixture substring is stripped from that
// file's content before pattern matching, so anything else in the file is still scanned.
// Strategic:
...
// Functional: each known value is built by concatenating two string literals split
// exactly at the "://" boundary, rather than written as one contiguous literal.
// Strategic: this file is itself a tracked, scanned file. A single contiguous literal
// here would make the scanner's own source match its own credential-shaped pattern on
// a default (non-explicit) scan of the tracked tree -- the split is not obfuscation of
// a real secret (these are public placeholders/fixtures, safe to read in full one line
// below), it exists purely so the source text has no unbroken regex-matching substring.
const DEFAULT_SCAN_VALUE_ALLOW_LIST = new Map([
  ['.env.example', ['mongodb+srv:' + '//username:password@cluster.mongodb.net/']],
  [
    'scripts/__fixtures__/secret-scan-positive.txt',
    ['mongodb+srv:' + '//fakeuser:fakepassword123@fixture-cluster.example.mongodb.net'],
  ],
]);
...
function main() {
  ...
  for (const file of files) {
    ...
    // Functional: strip only the exact known placeholder/fixture value(s) for this
    // file before scanning — never skip the file's content wholesale, so a real
    // credential pasted anywhere else in the same file is still caught.
    if (readMode !== 'explicit' && DEFAULT_SCAN_VALUE_ALLOW_LIST.has(file)) {
      for (const knownValue of DEFAULT_SCAN_VALUE_ALLOW_LIST.get(file)) {
        content = content.split(knownValue).join('');
      }
    }
